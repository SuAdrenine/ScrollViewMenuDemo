...
NSString * const tpHttpBaseUrl = @"https://api.tianapi.com/";

BOOL
judgeResultState(id object){
    BOOL success = NO;
    if ([object isKindOfClass:[NSDictionary class]]) {
        NSInteger result = [object[@"code"] integerValue];
        NSString *returnFlag = object[@"message"];
        if (result == 200 || [returnFlag isEqualToString:@"success"]) {
            success = YES;
        } else {
